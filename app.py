...
def fetch_nba_games_today():
    ...
    for g in games:
        ...
        # Fetch team stats for season records
        home_wins = home_t.get("statistics", [{}])[0].get("wins", {}).get("total") or 0 if home_t.get("statistics") else 0
        away_wins = away_t.get("statistics", [{}])[0].get("wins", {}).get("total") or 0 if away_t.get("statistics") else 0

        hp = 0.58  # NBA home advantage ~58%
        ap = 0.42
        events.append({
...
def fetch_nfl_games_today():
    ...
    for g in games:
        ...
        if not hn or not an:
            continue
        start = (g.get("game") or {}).get("date", {}).get("date") or ""
        if not is_valid_date(start):
            continue
        events.append({
...
def calculate_probs(fixture_data, prediction, injuries, h2h,
    ...
    lu_adj  = 0.02 if home_lu["confirmed"] else 0.0
