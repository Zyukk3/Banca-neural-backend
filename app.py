...
AF_H    = {"x-apisports-key": APIFOOTBALL_KEY}
...
NBA_H    = {"x-apisports-key": APIFOOTBALL_KEY}
NFL_H    = {"x-apisports-key": APIFOOTBALL_KEY}
MLB_H    = {"x-apisports-key": APIFOOTBALL_KEY}
NHL_H    = {"x-apisports-key": APIFOOTBALL_KEY}
...
def is_valid_date(ds):
    ...
    try:
        ...
        # Only matches that start from now onwards (max 7 days ahead)
        # No past matches allowed - strict future only
        return timedelta(minutes=-30) <= (d - now) <= timedelta(days=DATE_WINDOW)
    except Exception:
        return False

def is_today_or_future(ds):
    """Strict filter - only today and future, no yesterday"""
    if not ds:
        return False
    try:
        d = datetime.fromisoformat(str(ds).replace("Z", "+00:00"))
        ...
        return d >= today_start and (d - now) <= timedelta(days=DATE_WINDOW)
    except Exception:
        return False
...
def fetch_fixtures_today():
    ...
    data  = af_get("/fixtures", {"date": today, "status": "NS"})
...
def fetch_nba_games_today():
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    if not use_budget("nba", 1):
        return []
    data = safe_get(NBA_BASE + "/games", NBA_H, {"date": today})
...
def fetch_nfl_games_today():
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    if not use_budget("nfl", 1):
        return []
    data   = safe_get(NFL_BASE + "/games", NFL_H, {"date": today})
    games  = data.get("response") or []
    events = []
    for g in games:
...
def fetch_mlb_games_today():
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    if not use_budget("mlb", 1):
        return []
    data   = safe_get(MLB_BASE + "/games", MLB_H, {"date": today})
    games  = data.get("response") or []
    events = []
    for g in games:
        status = (g.get("status") or {}).get("short") or ""
...
def fetch_nhl_games_today():
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    if not use_budget("nhl", 1):
        return []
    data   = safe_get(NHL_BASE + "/games", NHL_H, {"date": today})
    games  = data.get("response") or []
    events = []
    for g in games:
        status = (g.get("status") or {}).get("short") or ""
...
def get_signals():
    global BUDGET
    BUDGET = {"football": {"used": 0, "limit": 90},
              "nba": {"used": 0, "limit": 20},
              "nfl": {"used": 0, "limit": 20},
              "mlb": {"used": 0, "limit": 20},
              "nhl": {"used": 0, "limit": 20}}
...
def debug():
    ...
    return jsonify({
        ...
        "budget":            BUDGET,
        "sample":            fixtures[:1] if fixtures else [],
    })
