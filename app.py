...
AF_H    = {"x-apisports-key": APIFOOTBALL_KEY, "x-rapidapi-host": "v3.football.api-sports.io", "x-rapidapi-key": APIFOOTBALL_KEY}

# API-Sports other sports (same key, 100 req/day each)
NBA_BASE = "https://v2.nba.api-sports.io"
NFL_BASE = "https://v1.american-football.api-sports.io"
MLB_BASE = "https://v1.baseball.api-sports.io"
NHL_BASE = "https://v1.hockey.api-sports.io"
NBA_H    = {"x-apisports-key": APIFOOTBALL_KEY, "x-rapidapi-host": "v2.nba.api-sports.io",               "x-rapidapi-key": APIFOOTBALL_KEY}
NFL_H    = {"x-apisports-key": APIFOOTBALL_KEY, "x-rapidapi-host": "v1.american-football.api-sports.io",  "x-rapidapi-key": APIFOOTBALL_KEY}
MLB_H    = {"x-apisports-key": APIFOOTBALL_KEY, "x-rapidapi-host": "v1.baseball.api-sports.io",           "x-rapidapi-key": APIFOOTBALL_KEY}
NHL_H    = {"x-apisports-key": APIFOOTBALL_KEY, "x-rapidapi-host": "v1.hockey.api-sports.io",             "x-rapidapi-key": APIFOOTBALL_KEY}
...
BUDGET = {
    "football": {"used": 0, "limit": 94},  # 100/day - 6 safety margin
    "nba":      {"used": 0, "limit": 95},  # 100/day - 5 safety margin
    "nfl":      {"used": 0, "limit": 95},
    "mlb":      {"used": 0, "limit": 95},
    "nhl":      {"used": 0, "limit": 95},
}
...
def fetch_nba_games_today():
    ...
    if not use_budget("nba", 3):
        return []
    games = []
    for delta in range(0, 3):
        day = (datetime.now(timezone.utc) + timedelta(days=delta)).strftime("%Y-%m-%d")
        data = safe_get(NBA_BASE + "/games", NBA_H, {"date": day, "season": AF_SEASON})
        resp = data.get("response") or []
        if not resp:
            # fallback without season param
            data = safe_get(NBA_BASE + "/games", NBA_H, {"date": day})
            resp = data.get("response") or []
        games.extend(resp)
    events = []
    for g in games:
...
def fetch_nfl_games_today():
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    if not use_budget("nfl", 3):
        return []
    games = []
    for delta in range(0, 3):
        day = (datetime.now(timezone.utc) + timedelta(days=delta)).strftime("%Y-%m-%d")
        data = safe_get(NFL_BASE + "/games", NFL_H, {"date": day, "season": AF_SEASON})
        resp = data.get("response") or []
        if not resp:
            data = safe_get(NFL_BASE + "/games", NFL_H, {"date": day})
            resp = data.get("response") or []
        games.extend(resp)
    events = []
    for g in games:
        status = (g.get("game") or {}).get("status", {}).get("short") or ""
        if status in ("FT", "HT", "Q1", "Q2", "Q3", "Q4", "OT"):
            continue
        teams  = g.get("teams") or {}
        home_t = teams.get("home") or {}
        away_t = teams.get("away") or {}
        ...
        if not hn or not an:
            continue
        game_obj = g.get("game") or {}
        date_obj  = game_obj.get("date") or {}
        start = date_obj.get("date") or date_obj.get("start") or game_obj.get("date") or g.get("date") or ""
        if not is_today_or_future(start):
            continue
        events.append({
            "id": str(g.get("game", {}).get("id") or ""),
            "home": hn, "away": an,
            "sport": "football", "league": "NFL", "start": start,
            "home_p": 0.57, "away_p": 0.43, "draw_p": None,
            "btts_p": None, "over25_p": None, "under25_p": None,
            "over15_p": None, "under15_p": None,
            "data_confidence": 60, "factors_used": ["nfl_api"],
            "api_advice": "", "api_winner": "",
            "api_home_odds": None, "api_draw_odds": None, "api_away_odds": None,
            "source": "NFL-API",
        })
...
def fetch_mlb_games_today():
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    if not use_budget("mlb", 3):
        return []
    games = []
    for delta in range(0, 3):
        day = (datetime.now(timezone.utc) + timedelta(days=delta)).strftime("%Y-%m-%d")
        data = safe_get(MLB_BASE + "/games", MLB_H, {"date": day, "season": AF_SEASON})
        resp = data.get("response") or []
        if not resp:
            data = safe_get(MLB_BASE + "/games", MLB_H, {"date": day})
            resp = data.get("response") or []
        games.extend(resp)
    events = []
    for g in games:
...
def fetch_nhl_games_today():
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    if not use_budget("nhl", 3):
        return []
    games = []
    for delta in range(0, 3):
        day = (datetime.now(timezone.utc) + timedelta(days=delta)).strftime("%Y-%m-%d")
        data = safe_get(NHL_BASE + "/games", NHL_H, {"date": day, "season": AF_SEASON})
        resp = data.get("response") or []
        if not resp:
            data = safe_get(NHL_BASE + "/games", NHL_H, {"date": day})
            resp = data.get("response") or []
        games.extend(resp)
    events = []
    for g in games:
        status = (g.get("status") or {}).get("short") or ""
        if status in ("FT", "AOT", "P1", "P2", "P3", "OT"):
            continue
        teams  = g.get("teams") or {}
        home_t = teams.get("home") or {}
        away_t = teams.get("away") or {}
        ...
        events.append({
            "id": str(g.get("id") or ""),
            "home": hn, "away": an,
            "sport": "hockey", "league": "NHL", "start": start,
            "home_p": 0.55, "away_p": 0.45, "draw_p": None,
            "btts_p": None, "over25_p": None, "under25_p": None,
            "over15_p": None, "under15_p": None,
            "data_confidence": 60, "factors_used": ["nhl_api"],
            "api_advice": "", "api_winner": "",
            "api_home_odds": None, "api_draw_odds": None, "api_away_odds": None,
            "source": "NHL-API",
        })
...
def get_signals():
    global BUDGET
    BUDGET = {"football": {"used": 0, "limit": 94},
              "nba": {"used": 0, "limit": 95},
              "nfl": {"used": 0, "limit": 95},
              "mlb": {"used": 0, "limit": 95},
              "nhl": {"used": 0, "limit": 95}}
    ...
    if fixtures:
        ...
        for fix_data in fixtures:
            ...
            if not fid:
                continue
            # 6 requests per match: prediction, injuries, h2h, home_stats, away_stats, lineups
            if BUDGET["football"]["used"] + 6 > BUDGET["football"]["limit"]:
                break
            # Also skip if fixture already started
    ...
    # 2. Other sports via API-Sports
    nba = fetch_nba_games_today()
    if nba:
        all_events.extend(nba)
        sources_used.append("API-Sports NBA")

    nfl = fetch_nfl_games_today()
    if nfl:
        all_events.extend(nfl)
        sources_used.append("API-Sports NFL")

    mlb = fetch_mlb_games_today()
    if mlb:
        all_events.extend(mlb)
        sources_used.append("API-Sports MLB")

    nhl = fetch_nhl_games_today()
    if nhl:
        all_events.extend(nhl)
        sources_used.append("API-Sports NHL")
...
def debug():
    ...
    espn     = fetch_espn_events()
    # Test each sport API
    nba_test = safe_get(NBA_BASE + "/games", NBA_H, {"date": datetime.now(timezone.utc).strftime("%Y-%m-%d")})
    mlb_test = safe_get(MLB_BASE + "/games", MLB_H, {"date": datetime.now(timezone.utc).strftime("%Y-%m-%d")})
    nhl_test = safe_get(NHL_BASE + "/games", NHL_H, {"date": datetime.now(timezone.utc).strftime("%Y-%m-%d")})
    nfl_test = safe_get(NFL_BASE + "/games", NFL_H, {"date": datetime.now(timezone.utc).strftime("%Y-%m-%d")})
    return jsonify({
        ...
        "api_tests": {
            ...
            "nhl": {"count": len(nhl_test.get("response") or []), "errors": nhl_test.get("errors")},
            "nfl": {"count": len(nfl_test.get("response") or []), "errors": nfl_test.get("errors")},
        },
