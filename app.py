...
def af_headers():
    k = get_key("APIFOOTBALL_KEY")
    return {"x-apisports-key": k, "x-rapidapi-host": "v3.football.api-sports.io", "x-rapidapi-key": k}

def nba_headers():
    k = get_key("APIFOOTBALL_KEY")
    return {"x-apisports-key": k, "x-rapidapi-host": "v2.nba.api-sports.io", "x-rapidapi-key": k}

def mlb_headers():
    k = get_key("APIFOOTBALL_KEY")
    return {"x-apisports-key": k, "x-rapidapi-host": "v1.baseball.api-sports.io", "x-rapidapi-key": k}
...
def blend(stats, groq, w=0.35):
    ...
    dp = stats.get("draw_p",0)*sw + groq.get("prob_draw",stats.get("draw_p",0))*w
    t  = hp+ap+dp
    r  = dict(stats)
    r["home_p"] = round(hp/t, 4)
    r["away_p"] = round(ap/t, 4)
    r["draw_p"] = round(dp/t, 4)
    r["groq_ok"]      = True
...
def process_fixture(fix_data, season):
    """Process one fixture: fetch all data and compute probs. Max 5 API calls."""
    ...
    # Parallel fetch of all sub-data
    pred, injuries, h2h, hs, as_, lineups = {}, [], [], {}, {}, []
    with ThreadPoolExecutor(max_workers=6) as ex:
        f_pred    = ex.submit(fetch_prediction, fid)
        f_inj     = ex.submit(fetch_injuries, fid)
        f_h2h     = ex.submit(fetch_h2h, hid, aid) if hid and aid else None
        f_hs      = ex.submit(fetch_team_stats, hid, lid, season) if hid and lid else None
        f_as      = ex.submit(fetch_team_stats, aid, lid, season) if aid and lid else None
        f_lu      = ex.submit(fetch_lineups, fid)

        try: pred     = f_pred.result(timeout=9)
        except: pass
        try: injuries = f_inj.result(timeout=9)
        except: pass
        try: h2h      = f_h2h.result(timeout=9) if f_h2h else []
        except: pass
        try: hs       = f_hs.result(timeout=9) if f_hs else {}
        except: pass
        try: as_      = f_as.result(timeout=9) if f_as else {}
        except: pass
        try: lineups  = f_lu.result(timeout=9)
        except: pass
...
def get_signals():
    ...
    if fixtures:
        ...
        MAX_FIXTURES = 8  # limit to avoid timeout
...
def debug():
    ...
    return jsonify({
        ...
        "football_sample":[{"home":(f.get("teams") or {}).get("home",{}).get("name"),
                            ...
                            "start":(f.get("fixture") or {}).get("date")} for f in fixtures[:3]],
    })
