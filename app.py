...
def build_signals(event, odds_game):
    ...
    has_real_odds = len(bks) > 0  # Only trust real bookmaker odds
    sigs = []

    def add(label, mkt, outcome, prob, odds, mtype, real_odds=False):
        if not prob or prob<=0 or odds<=1.01: return
        # If no real odds and odds are estimated, require higher prob threshold
        if not real_odds and not has_real_odds:
            if prob < 0.65:  # Much stricter - only very high confidence
                return
        ...
        sigs.append({
            ...
            "real_odds": real_odds or has_real_odds,
            "_p":prob,"_o":odds,
        })
    ...
    # 1X2 - only with real odds
    if has_real_odds:
        ho = best_odds(bks,"h2h",hn)
        ao = best_odds(bks,"h2h",an)
        if ho > 1.01: add(hn+" GANA","h2h",hn,hp,ho,"1X2",True)
        if ao > 1.01: add(an+" GANA","h2h",an,ap,ao,"1X2",True)
        if dp is not None:
            do = best_odds(bks,"h2h","Draw")
            if do > 1.01: add("EMPATE","h2h","Draw",dp,do,"1X2",True)
    else:
        # No real odds - use estimated but only for very strong favorites
        ho = round(1.0/max(hp,0.01)*1.05,2)
        ao = round(1.0/max(ap,0.01)*1.05,2)
        add(hn+" GANA","h2h",hn,hp,ho,"1X2",False)
        add(an+" GANA","h2h",an,ap,ao,"1X2",False)

    # Double chance - only with real odds
    if has_real_odds and dp is not None:
        p1x=hp+dp; px2=ap+dp; p12=hp+ap
        o1x=best_odds(bks,"double_chance","1X")
        ox2=best_odds(bks,"double_chance","X2")
        o12=best_odds(bks,"double_chance","12")
        if o1x>1.01: add(hn+" o Empate (1X)","double_chance","1X",p1x,o1x,"Doble Oportunidad",True)
        if ox2>1.01: add(an+" o Empate (X2)","double_chance","X2",px2,ox2,"Doble Oportunidad",True)
        if o12>1.01: add(hn+" o "+an+" (12)","double_chance","12",p12,o12,"Doble Oportunidad",True)

    # BTTS - only with real odds
    bp=event.get("btts_p")
    if bp and has_real_odds:
        bo=best_odds(bks,"btts","Yes")
        bn=best_odds(bks,"btts","No")
        if bo>1.01: add("AMBOS MARCAN - Si","btts","Yes",bp,bo,"BTTS",True)
        if bn>1.01: add("AMBOS MARCAN - No","btts","No",1-bp,bn,"BTTS",True)

    # Over/Under - use real odds when available
    for line,lo,lu in [(2.5,"MAS DE 2.5 GOLES","MENOS DE 2.5 GOLES"),
        ...
        if has_real_odds:
            # Use real odds from bookmakers
            oo = best_odds(bks,"totals","Over")
            uo = best_odds(bks,"totals","Under")
            if oo>1.01: add(lo,"totals","Over",op,oo,"Over/Under",True)
            if uo>1.01 and up: add(lu,"totals","Under",up,uo,"Over/Under",True)
        else:
            # No real odds - only recommend if probability is very high
            est_oo = round(1/max(op,.01)*1.05,2)
            est_uo = round(1/max(up or .01,.01)*1.05,2)
            add(lo,"totals","Over",op,est_oo,"Over/Under",False)
            if up: add(lu,"totals","Under",up,est_uo,"Over/Under",False)
...
def generate_recommendation(sigs, event):
    """
    From all valid signals for a match, pick the BEST one and generate
    ...
    if ev >= 15 and prob >= 65:
        nivel = "MUY ALTA"
        emoji_nivel = "FUEGO"
    elif ev >= 8 and prob >= 58:
        nivel = "ALTA"
        emoji_nivel = "OK"
    elif ev >= 4 and prob >= 52:
        nivel = "MEDIA"
        emoji_nivel = "RAYO"
    else:
        nivel = "BAJA"
    ...
    reasons = []
    if prob >= 70:
        reasons.append(f"Alta probabilidad estadistica ({prob:.0f}%)")
    if ev >= 10:
        reasons.append(f"Valor esperado elevado (+{ev:.1f}%)")
    if odds >= 1.80:
        reasons.append(f"Cuota atractiva ({odds})")
    if best.get("real_odds"):
        reasons.append("Cuota verificada en bookmakers reales")
    if not reasons:
        reasons.append(f"EV positivo con probabilidad {prob:.0f}%")
    ...
    best["todas_opciones"]  = [
        ...
        for s in sorted(valid, key=lambda x: x["score"], reverse=True)[:5]
    ]
