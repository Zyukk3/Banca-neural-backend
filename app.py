...
def build_signals(event, odds_game):
    """
    Build betting signals ONLY from real bookmaker odds.
    Never generates signals from estimated odds - that causes systematic bias.
    """
    bks = odds_game.get("bookmakers",[]) if odds_game else []

    # NO real odds = NO signals. Period.
    if not bks:
        return []
    ...
    if ho > 1.01: add(hn+" GANA","h2h",hn,hp,ho,"1X2")
    if ao > 1.01: add(an+" GANA","h2h",an,ap,ao,"1X2")
    if dp is not None:
        do = best_odds(bks,"h2h","Draw")
        if do > 1.01: add("EMPATE","h2h","Draw",dp,do,"1X2")

    # Double chance with real odds
    if dp is not None:
        ...
        if o1x>1.01: add(hn+" o Empate (1X)","double_chance","1X",p1x,o1x,"Doble Oportunidad")
        if ox2>1.01: add(an+" o Empate (X2)","double_chance","X2",px2,ox2,"Doble Oportunidad")
        if o12>1.01: add(hn+" o "+an+" (12)","double_chance","12",p12,o12,"Doble Oportunidad")
    ...
    if bp:
        ...
        if bo>1.01: add("AMBOS MARCAN - Si","btts","Yes",bp,bo,"BTTS")
        if bn>1.01: add("AMBOS MARCAN - No","btts","No",1-bp,bn,"BTTS")

    # Over/Under with real odds - each line separately
    bp25 = event.get("over25_p"); bm25 = event.get("under25_p")
    bp15 = event.get("over15_p"); bm15 = event.get("under15_p")
    bp35 = event.get("over35_p"); bm35 = event.get("under35_p")

    # Get real odds for each line from bookmakers
    for line, op, up, lo_label, lu_label in [
        (2.5, bp25, bm25, "MAS DE 2.5 GOLES",  "MENOS DE 2.5 GOLES"),
        (1.5, bp15, bm15, "MAS DE 1.5 GOLES",  "MENOS DE 1.5 GOLES"),
        (3.5, bp35, bm35, "MAS DE 3.5 GOLES",  "MENOS DE 3.5 GOLES"),
    ]:
        if not op: continue
        # Find odds for this specific line in bookmakers
        over_o = 1.01; under_o = 1.01
        for bk in bks:
            for mkt in bk.get("markets",[]):
                if mkt["key"] == "totals":
                    for out in mkt.get("outcomes",[]):
                        ...
                        # Match line exactly (2.5, 1.5, 3.5)
                        if abs(pt - line) < 0.01 or pt == 0:
                            if "over" in name:
                                over_o = max(over_o, out.get("price",1.01))
                            elif "under" in name:
                                under_o = max(under_o, out.get("price",1.01))

        if over_o > 1.01:  add(lo_label,"totals",f"Over {line}",op,over_o,"Over/Under")
        if under_o > 1.01 and up: add(lu_label,"totals",f"Under {line}",up,under_o,"Over/Under")
...
def generate_recommendation(sigs, event):
    ...
    REQUIRES real bookmaker odds - never recommends based on estimated odds.
    """
    valid = [s for s in sigs
             if s["_p"] >= MIN_PROB
             and s["_o"] >= MIN_ODDS
             and s["ev_pct"] > 0
             and s.get("real_odds", False)]  # ONLY real bookmaker odds
